...
def send_text_dashboard(activityId,count,area,exceldata,even):
    ...
    # print(type(codes))
    if codes != 1:
        logger.error("--- 请确认 {} 大区文案未上传成功 (code -7  活动id 不存在，code 10 关键字包含dopalive)---".format(area))
...
if __name__ == '__main__':
    # 解析命令行参数
    parser = argparse.ArgumentParser(description='上传活动文案和检查礼物配置 (dopalive平台)')
    ...
    # print(logspath)
    logger.add(r'%s/dopalivesendtext_{time:YYYY-MM-DD}.log' % logspath,
               rotation="00:00")
    ...
    # 检查活动ID是否以3开头（dopalive平台）
    is_dopalive = str(activity_id).startswith('3') or activity_id >= 30000
    logger.info(f'--该活动ID是否属于dopalive平台: {is_dopalive}--')
