<template>
  <div class="swiper-component-container-wrap-rew" v-if="isShowSwiper && list.length > 0">
    <Swiper
      ...
      :observeParents="true"
    >
      <SwiperSlide :swiperRef="swiperRef" class="swiper-slide-self" v-for="(item, index) in list || []" :key="index">
...
const props = defineProps({
  ...
  slidesPerGroup: {
    type: Number,
    default: 0 // 0 表示使用默认值（跟 slidesPerView 一样）
  },
  forceLoop: {
    type: Boolean,
    default: false // 强制开启 loop 模式
  }
})
...
const shouldRoll = computed(() => props.forceLoop || (props.list?.length >= props.maxNumber && props.list?.length > 1))
...
const setSwiperOptions = () => {
  ...
  // 设置每次滚动的数量
  if (props.slidesPerGroup > 0) {
    baseOptions.slidesPerGroup = props.slidesPerGroup
  }

  swiper_options.value = {
    ...baseOptions,
    autoplay:
      props.list?.length < props.maxNumber && !props.forceLoop
        ? false
        : {
