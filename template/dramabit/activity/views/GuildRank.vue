<template>
  <div class="guild-rank" :class="{ coming: status === -1 }">
        ...
        >
          <template #default="{ list, status }">
            ...
            <Top3
              ...
              disableJump
              hideFlag
              @openMemberRank="handleOpenMemberRank"
            />
            <template
            ...
            >
              ...
              <RankDivider
                v-if="
                  otherParam === '1' && (showTop3 ? idx + 4 : idx + 1) === 10
                "
                :text="TOOL_TEXT[25]?.replace('X', 10)"
              />
            </template>
          </template>
...
import MemberRankDialog from "../components/RankList/MemberRankDialog.vue";
...
const api = "/api/activity/commonBusiness/unionTotalRank";
...
const tipText = computed(() => {
  const text = TOOL_TEXT[otherParam.value === "2" ? 24 : 21] || "";
  return text.replace(/，/g, "，<br>");
});

// 成员榜单弹框
const showMemberDialog = ref(false);
const currentGuild = ref<InfoType | null>(null);

const handleOpenMemberRank = (guild: InfoType) => {
