<!-- 描边组件示例 -->
<!--
  <Outline
    :color="1 ? '0.05rem #9CFFFB' : '0.025rem #FFFB9C'"
    :text="1"
  />
-->

<script lang="ts">
export default defineComponent({
  name: 'Outline',
  components: {},
  props: {
    color: {
      type: String,
      default: '0.05rem #f40'
    },
    text: {
      type: [String, Number],
      default: '---'
    },
    noColor: {
      type: Boolean,
      default: false
    },
    // 溢出样式 props（优先级高于 class 判断）
    overflow: {
      type: String,
      default: undefined
    },
    textOverflow: {
      type: String,
      default: undefined
    },
    whiteSpace: {
      type: String,
      default: undefined
    }
  },
  setup(props) {
    const cur: any = getCurrentInstance()
    const isOverflow = cur?.attrs?.class?.split(' ').find((item) => item == 'ov' || item == 'ow')
    // props 优先，否则根据 class 判断
    const o = props.overflow ?? (isOverflow ? 'hidden' : 'visible')
    const t = props.textOverflow ?? (isOverflow ? 'ellipsis' : 'clip')
    const w = props.whiteSpace ?? (isOverflow ? 'nowrap' : 'normal')
    return { o, t, w, ...toRefs(props) }
  },
  render() {
    return (
      this.text !== null &&
      this.text !== undefined &&
      h('span', { class: this.noColor ? '' : 'text-outline', 'data-text': this.text }, this.text)
    )
  }
})
</script>

<style lang="scss" scoped>
.text-outline {
  position: relative;
  z-index: 1;
  display: inline-block;
  -webkit-text-stroke-width: 0 !important;
  -webkit-text-stroke-color: unset !important;

  &::before {
    content: attr(data-text);
    width: 100%;
    height: 100%;
    display: block;
    position: absolute;
    z-index: -1;
    -webkit-text-stroke: v-bind(color);
    text-stroke: v-bind(color);
    overflow: v-bind(o);
    text-overflow: v-bind(t);
    white-space: v-bind(w);
  }
}
</style>
